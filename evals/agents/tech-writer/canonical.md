...
Snapshot evaluation for the `tech-writer` agent. Run via `bash evals/run.sh tech-writer`.
...
```
(operator prompt that should trigger @tech-writer)
```
...
- Output contract sections per `config/agents/tech-writer.md`
