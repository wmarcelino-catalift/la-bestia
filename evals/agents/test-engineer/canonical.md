...
Snapshot evaluation for the `test-engineer` agent. Run via `bash evals/run.sh test-engineer`.
...
```
(operator prompt that should trigger @test-engineer)
```
...
- Output contract sections per `config/agents/test-engineer.md`
