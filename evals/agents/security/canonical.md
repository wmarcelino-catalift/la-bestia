...
Snapshot evaluation for the `security` agent. Run via `bash evals/run.sh security`.
...
```
(operator prompt that should trigger @security)
```
...
- Output contract sections per `config/agents/security.md`
