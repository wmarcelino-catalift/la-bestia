...
Snapshot evaluation for the `code-reviewer` agent. Run via `bash evals/run.sh code-reviewer`.
...
```
(operator prompt that should trigger @code-reviewer)
```
...
- Output contract sections per `config/agents/code-reviewer.md`
