...
echo ""
echo "[Structure: $CDIR]"
check "agents/ (>= 12)"         "[ \$(ls \"$CDIR/agents\"/*.md 2>/dev/null | wc -l) -ge 12 ]"
check "skills/ (>= 4)"          "[ \$(ls -d \"$CDIR/skills\"/*/ 2>/dev/null | wc -l) -ge 4 ]"
check "commands/ (>= 11)"       "[ \$(ls \"$CDIR/commands\"/*.md 2>/dev/null | wc -l) -ge 11 ]"
check "hooks/ (>= 5)"           "[ \$(ls \"$CDIR/hooks\"/*.sh 2>/dev/null | wc -l) -ge 5 ]"
check "scripts/ (>= 3)"         "[ \$(ls \"$CDIR/scripts\"/*.sh 2>/dev/null | wc -l) -ge 3 ]"
check "bin/ (>= 3)"             "[ \$(ls \"$CDIR/bin\"/*.sh 2>/dev/null | wc -l) -ge 3 ]"
