...
check "agents/ (>=12)"        "[ \$(ls \"$PROJ/.claude/agents\"/*.md 2>/dev/null | wc -l) -ge 12 ]"
check "skills/ (>=3)"         "[ \$(ls -d \"$PROJ/.claude/skills\"/*/ 2>/dev/null | wc -l) -ge 3 ]"
check "commands/ (>=9)"       "[ \$(ls \"$PROJ/.claude/commands\"/*.md 2>/dev/null | wc -l) -ge 9 ]"
...
check "agent-memory/ (>=11)"  "[ \$(ls -d \"$PROJ/.claude/agent-memory\"/*/ 2>/dev/null | wc -l) -ge 11 ]"
check "vault/ stub"           "[ -f \"$PROJ/.claude/vault/HOT.md\" ]"
check "memory/hot-context"    "[ -f \"$PROJ/memory/hot-context.md\" ]"
check "memory/decisions/"     "[ -d \"$PROJ/memory/decisions\" ]"
check ".claudeignore"         "[ -f \"$PROJ/.claudeignore\" ]"
check "devops agent"          "[ -f \"$PROJ/.claude/agents/devops.md\" ]"
check "mobile-reviewer agent" "[ -f \"$PROJ/.claude/agents/mobile-reviewer.md\" ]"
check "content-manager agent" "[ -f \"$PROJ/.claude/agents/content-manager.md\" ]"
check "ux-reviewer agent"     "[ -f \"$PROJ/.claude/agents/ux-reviewer.md\" ]"
check "pm agent"              "[ -f \"$PROJ/.claude/agents/pm.md\" ]"
check "data-engineer agent"   "[ -f \"$PROJ/.claude/agents/data-engineer.md\" ]"
